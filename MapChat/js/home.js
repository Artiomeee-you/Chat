 ...
 sizeInput.addEventListener('input', e => {
  ...
  photoInput.onchange = (event) => {
    ...
    if(file){
      ...
      uploadPhotoButton.textContent = `Выбрано: ${file.name}`;
    }
  };
...
if (profileCard) {
  profileCard.classList.toggle('gender-male', d.gender === 'male');
  profileCard.classList.toggle('gender-female', d.gender === 'female');
}
...
function save(name, age, gender, msg, lat, lng) {
  set(userRef, {
   ...
   .catch(err => {
     // выведем полную ошибку в консоль
     console.error('Ошибка сохранения профиля:', err);
     // и покажем пользователю, что именно не так
...
genderSelect.addEventListener('change', () => {
  ...
  card.classList.toggle('gender-male', genderSelect.value === 'male');
  card.classList.toggle('gender-female', genderSelect.value === 'female');
});
